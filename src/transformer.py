...
class Transformer(nn.Module):
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            h_v_norm = torch.nn.functional.normalize(h_v, p=2, dim=-1)  # Shape: (k, hidden_size)
            h_j_norm = torch.nn.functional.normalize(h_j, p=2, dim=-1)  # Shape: (k, seq_len, hidden_size)
            ...
            best_token = topk_indices[torch.arange(batch_size, device=device), best_idx].unsqueeze(-1)  # Shape: (1, 1)
